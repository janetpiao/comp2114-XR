...
const createScene = async function() {
    ...
    // lab-b addition starts here
    // Added a star
    const star = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "star.glb").then((result) => {
        // Do this stuff after the mesh has been loaded
        const starMesh = result.meshes[0];
        // Grab the bounding box
        // const starBounds = result.meshes[1];
        // starBounds.showBoundingBox = true;
        // wheel1Mesh.position = new BABYLON.Vector3(0.5, 0.2, -1.5);
        starMesh.scaling = new BABYLON.Vector3(10, 10, 10);
        
        // Position
        starMesh.position = new BABYLON.Vector3(-0.5, 3.5, -6);
    }).catch((error) => {
        // Oops, the mesh didn't load for some reason
        console.error("Error loading mesh:" + error);
        return null;
    ...
    // Added a cat
    const cat = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "cat.glb").then((result) => {
        // Do this stuff after the mesh has loaded
        const catMesh = result.meshes[0];
        // Grab the bouding box
        // const catBounds = result.meshes[1];
        // catBounds.showBoundingBox = true;
        // wheelMesh.position = new BABYLON.Vector3(0, 0, 0);
        catMesh.scaling = new BABYLON.Vector3(0.25, 0.25, 0.25);
        // wheelMesh.rotate.y = BABYLON.Tools.ToRadians(90);
        // Attach the wheel to the cart mesh (parent)
        catMesh.parent = cart;
        // Position wheel with respect to centre of cart mesh
        catMesh.position = new BABYLON.Vector3(-5, -0.85, -0.75);
    }).catch((error) => {
        // Oops, the mesh didn't load for some reason
        console.error("Error loading mesh: " + error);
        return null;
    ...
    // Added a snowman
    const snowman = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "snowman.glb").then((result) => {
        // Do this stuff after the mesh has loaded
        const snowmanMesh = result.meshes[0];
        // Grab the bounding box
        // const snowmanBounds = result.meshes[1];
        // snowmanBounds.showBoundingBox = true;
        // wheelMesh.position = new BABYLON.Vector3(0, 0, 0);
        snowmanMesh.scaling = new BABYLON.Vector3(5,5,5);
        // wheelMesh.rotate.y = BABYLON.Tools.ToRadians(90);
        // Attach the wheel to the cart mesh (parent)
        snowmanMesh.parent = cart;
        // Position wheel with respect to centre of cart mesh
        snowmanMesh.position = new BABYLON.Vector3(-1.5, -0.85, -0.75);
    }).catch((error) => {
        // Oops, the mesh didn't load for some reason
        console.error("Error loading mesh: " + error);
        return null;
